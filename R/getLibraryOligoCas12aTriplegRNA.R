...
#'@param pos1spacer,pos2spacer,pos3spacer string containing only spacer DNA sequences of length 19-23nt. Must not contain BsmbI sites. Must not contain TTTT. Must not start with TTT, because the last base of each DR is a T and that would form an undesirable TTTT at the junction.
#'@param pos1name,pos2name,pos3name string containing name for each spacer.
...
#'@export
#'@examples getLibraryOligoCas12aTriplegRNA( pos1name = 'CD81-1', pos1spacer = 'GGCGCGACCCCCAGGAAGGTCTC', pos2name = 'B2M-1', pos2spacer = 'ATATAAGTGGAGGCGTCGCGCTG', pos3name = 'KIT-2', pos3spacer = 'TCTGCGTTCTGCTCCTACTGCTT', Adaptor5p = 'ATTTTGCCCCTGGTTCTT', Adaptor3p = 'CCAGTTCATTTCTTAGGG')
...
getLibraryOligoCas12aTriplegRNA <- function( pos1name,
      ...
      assertthat::assert_that(
            is.character( c( pos1name, pos2name, pos3name ) ), msg = 'names must be character' )
      ...
      transcript <- stringr::str_c( pos1spacer, DR1, pos2spacer, DR3, pos3spacer )
      ...
      allspacers <- c(pos1spacer, pos2spacer, pos3spacer )
