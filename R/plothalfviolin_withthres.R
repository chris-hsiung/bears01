...
plothalfviolin_withthres <- function(data, x_var, y_var, fill_var, dodge_width = 0.8, color_palette = c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7"), facetformula_var, title_var, thres_var, perc_var, perclabelx = 1, labelsize = 6, basesize_var = 18, nudge_var = 0.5) {
      ...
      thresdf <- dplyr::select(data, all_of(c(y_var, thres_var, facet_strvar, fill_var))) %>%
            unique()
      # dplyr::mutate( y_varnumeric = seq(1,length(unique(.data[[y_var]]))) )

      yvarnumeric <- seq( 1, length(unique(data[[y_var]])), 1)

      yvarnumericdf <- data.frame( unique(data[[y_var]]), yvarnumeric, yvarnumeric+1 )
      ...
      plotout <- ggplot() +
            ...
            stat_summary(data = data, aes_string(y = y_var, x = x_var), fun = 'mean',
                         fun.max = function(x) { quantile(x, 0.25) },
                         fun.min = function(x) { quantile(x, 0.75) },
                         geom = "pointrange", show.legend = FALSE, position = position_nudge(x = 0, y = 0.35)) +
