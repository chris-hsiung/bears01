#' t Test for Two Groups in a DataFrame
#'
#' Performs a t test on two groups within a dataframe based on a grouping variable and returns the p-value.
#'
...
#' @param value_var The column name in dataframe representing the values on which the t test is performed.
...
#'
#' @return A data frame with the p-value of the t test. If there's insufficient data for either group, the result will be "Insufficient data".
#'
#' @examples
#' \dontrun{
...
#'
#' @export
ttestfordf <- function(dataframe, groupvar1, groupvar2, groupname_var, value_var, alternative_var, round_var = 2, sciformat_var = FALSE){
  ...
  if ( sum( !(is.na(groupvar1df[[value_var]])) ) < 2 | sum( !(is.na(groupvar2df[[value_var]])) ) < 2) {
    ...
    pvalue <- format(t.test(groupvar1df[[value_var]], groupvar2df[[value_var]], alternative = alternative_var)$p.value, scientific = sciformat_var, digits = round_var)
  }

  dfout <- data.frame( ttest_pvalue = as.numeric(pvalue))
  return(dfout)
